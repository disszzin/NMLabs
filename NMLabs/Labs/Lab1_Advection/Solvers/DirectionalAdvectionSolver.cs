...
public class DirectionalAdvectionSolver : IMathSolver<AdvectionParameters, (double[] X, List<double[]> History)>
{
    public (double[] X, List<double[]> History) Solve(AdvectionParameters data)
    {
        int n = (int)(data.X / data.Dx) + 1;    // Число элементов в массивах
        double dt = data.CFL_Directional * data.Dx / data.U;    // Шаг по времени
        ...
        {
            for (int i = 1; i < n - 1; i++) 
                T2[i] = T1[i] - data.U * dt / data.Dx * (T1[i] - T1[i - 1]);        // Расчет по формуле
